...
SHEET = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "public", "styles.css")
OPEN = "/* >>> generated by tools/make-assets.py, do not edit by hand <<< */"
CLOSE = "/* >>> end generated <<< */"
# A value too long for one line continues at this depth. Both the emitter
# and _stains wrap against it, so it lives here rather than in each of them.
VALUE_INDENT = "    "
...
def main():
    ...
    block = re.search(re.escape(OPEN) + ".*?" + re.escape(CLOSE), sheet, re.S)
    if not block:
    ...
    if "--check" in sys.argv:
        if block.group(0) != fresh:
            sys.exit("FAIL: %s is out of date. Run tools/make-assets.py" % SHEET)
        print("%s is up to date" % SHEET)
        return
    if block.group(0) == fresh:
        print("%s already up to date" % SHEET)
        return
    open(SHEET, "w", encoding="utf-8").write(sheet[:block.start()] + fresh
                                             + sheet[block.end():])
    print("rewrote the generated block in %s" % SHEET)
